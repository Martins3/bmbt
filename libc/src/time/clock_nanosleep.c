...
int __clock_nanosleep(clockid_t clk, int flags, const struct timespec *req,
  ...
  if (SYS_clock_nanosleep == SYS_clock_nanosleep_time64 || !IS32BIT(s))
    r = libc_syscall_cp(SYS_clock_nanosleep_time64, clk, flags,
                        ((long long[]){s, ns}), rem);
  ...
  if (clk == CLOCK_REALTIME && !flags)
    r = libc_syscall_cp(SYS_nanosleep, &ts32, &ts32);
  else
    r = libc_syscall_cp(SYS_clock_nanosleep, clk, flags, &ts32, &ts32);
...
#else
  if (clk == CLOCK_REALTIME && !flags)
    return -libc_syscall_cp(SYS_nanosleep, req, rem);
  return -libc_syscall_cp(SYS_clock_nanosleep, clk, flags, req, rem);
#endif
}
