            ...
            FullLoadHelper *full_load)
{
    ...
    if (btmmu_enabled() && !code_read) {
        ...
        if (prot & PROT_READ) {
            btmmu_map_page(env_cpu(env), vaddr_page, vaddr_page + entry->addend, mmu_idx, prot & PROT_WRITE ? 1 : 0);
        }
    }
#endif
...
tcg_target_ulong xtm_helper_ret_ldub_mmu(CPUArchState *env,
...
#ifdef CONFIG_X86toMIPS
tcg_target_ulong xtm_helper_le_lduw_mmu(CPUArchState *env,
        target_ulong addr, TCGMemOpIdx oi)
{
...
#ifdef CONFIG_X86toMIPS
tcg_target_ulong xtm_helper_le_ldul_mmu(CPUArchState *env,
        target_ulong addr, TCGMemOpIdx oi)
{
...
#ifdef CONFIG_X86toMIPS
uint64_t xtm_helper_le_ldq_mmu(CPUArchState *env,
        target_ulong addr, TCGMemOpIdx oi)
{
    return load_helper(env, addr, oi, GETPC(), MO_LEQ, false,
                       helper_le_ldq_mmu);
}
#endif
...
#ifdef CONFIG_X86toMIPS
tcg_target_ulong xtm_helper_ret_ldsb_mmu(CPUArchState *env,
        target_ulong addr, TCGMemOpIdx oi)
{
...
#ifdef CONFIG_X86toMIPS
tcg_target_ulong xtm_helper_le_ldsw_mmu(CPUArchState *env,
        target_ulong addr, TCGMemOpIdx oi)
{
...
#ifdef CONFIG_X86toMIPS
tcg_target_ulong xtm_helper_le_ldsl_mmu(CPUArchState *env,
        target_ulong addr, TCGMemOpIdx oi)
{
             ...
             TCGMemOpIdx oi, uintptr_t retaddr, MemOp op)
{
    ...
    if (btmmu_enabled()) {
        ...
        if (entry->addr_write == vaddr_page) prot |= PROT_WRITE; 
        if (prot & PROT_WRITE) {
            btmmu_map_page(env_cpu(env), vaddr_page, vaddr_page + entry->addend, mmu_idx, 1);
        }
    }
#endif
}
...
#ifdef CONFIG_X86toMIPS
void xtm_helper_ret_stb_mmu(CPUArchState *env,
        target_ulong addr, uint8_t val, TCGMemOpIdx oi)
{
...
#ifdef CONFIG_X86toMIPS
void xtm_helper_le_stw_mmu(CPUArchState *env,
        target_ulong addr, uint16_t val, TCGMemOpIdx oi)
{
...
#ifdef CONFIG_X86toMIPS
void xtm_helper_le_stl_mmu(CPUArchState *env,
        target_ulong addr, uint32_t val, TCGMemOpIdx oi)
{
...
#ifdef CONFIG_X86toMIPS
void xtm_helper_le_stq_mmu(CPUArchState *env,
        target_ulong addr, uint64_t val, TCGMemOpIdx oi)
{
