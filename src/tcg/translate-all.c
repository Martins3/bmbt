...
static gboolean x86_to_mips_free_etb(gpointer key, gpointer value, gpointer data)
{
...
static inline void tb_reset_jump(TranslationBlock *tb, int n)
{
    ...
    if (xtm_branch_opt() && n == 1 && tb->extra_tb->branch_to_target_direct_in_mips_branch == 1)
    {
        uint32_t *addr =
...
#ifdef CONFIG_X86toMIPS
    if (tb->extra_tb) tb->extra_tb->next_tb[n] = NULL;
#endif
}
...
#if defined(CONFIG_SOFTMMU) && defined(CONFIG_X86toMIPS) && defined(CONFIG_XTM_TEST)
...
void *get_code_highwater(void)
{
    return tcg_ctx->code_gen_highwater;
}
#endif
