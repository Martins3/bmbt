...
#ifdef CONFIG_PROFILER
  atomic_set(&prof->restore_time, prof->restore_time + profile_getclock() - ti);
...
static int walk_memory_regions_1(struct walk_memory_regions_data *data,
  ...
  if (level == 0) {
    ...
    for (i = 0; i < V_L2_SIZE; ++i) {
      pa = base | ((target_ulong)i << (TARGET_PAGE_BITS + V_L2_BITS * level));
...
static int dump_region(void *priv, target_ulong start, target_ulong end,
  ...
  (void)fprintf(
      f, TARGET_FMT_lx "-" TARGET_FMT_lx " " TARGET_FMT_lx " %c%c%c\n", start,
      end, end - start, ((prot & PAGE_READ) ? 'r' : '-'),
      ((prot & PAGE_WRITE) ? 'w' : '-'), ((prot & PAGE_EXEC) ? 'x' : '-'));
...
void page_set_flags(target_ulong start, target_ulong end, int flags) {
  ...
  for (addr = start, len = end - start; len != 0;
    ...
    /* If the write protection bit is set, then we invalidate
       the code inside.  */
    if (!(p->flags & PAGE_WRITE) && (flags & PAGE_WRITE) && p->first_tb) {
...
int page_unprotect(target_ulong address, uintptr_t pc) {
  ...
  if (p->flags & PAGE_WRITE_ORG) {
    ...
    if (p->flags & PAGE_WRITE) {
      ...
      mprotect((void *)g2h(host_start), qemu_host_page_size, prot & PAGE_BITS);
    }
    mmap_unlock();
