...
static inline struct qht_bucket *qht_map_to_bucket(const struct qht_map *map,
                                                   uint32_t hash) {
 ...
 */
static inline void qht_map_lock_buckets__no_stale(struct qht *ht,
                                                  struct qht_map **pmap) {
  struct qht_map *map;
...
qht_bucket_lock__no_stale(struct qht *ht, uint32_t hash,
                          struct qht_map **pmap) {
  struct qht_bucket *b;
  struct qht_map *map;
...
static struct qht_map *qht_map_create(size_t n_buckets) {
  ...
  map->n_added_buckets_threshold =
      n_buckets / QHT_NR_ADDED_BUCKETS_THRESHOLD_DIV;
...
static inline void qht_do_resize_and_reset(struct qht *ht,
                                           struct qht_map *new) {
...
static inline void *qht_do_lookup(const struct qht_bucket *head,
                                  qht_lookup_func_t func, const void *userp,
                                  uint32_t hash) {
...
qht_lookup__slowpath(const struct qht_bucket *b, qht_lookup_func_t func,
...
static void *qht_insert__locked(const struct qht *ht, struct qht_map *map,
  ...
  do {
    for (i = 0; i < QHT_BUCKET_ENTRIES; i++) {
      if (b->pointers[i]) {
        if (unlikely(b->hashes[i] == hash && ht->cmp(b->pointers[i], p))) {
          return b->pointers[i];
        }
      } else {
        goto found;
...
static void qht_entry_move(struct qht_bucket *to, int i,
                           struct qht_bucket *from, int j) {
...
static inline bool qht_remove__locked(struct qht_bucket *head, const void *p,
                                      uint32_t hash) {
  struct qht_bucket *b = head;
  int i;
...
static inline void do_qht_iter(struct qht *ht, const struct qht_iter *iter,
                               void *userp) {
  struct qht_map *map;
