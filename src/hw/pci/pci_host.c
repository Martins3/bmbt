...
void pci_host_config_write_common(PCIDevice *pci_dev, uint32_t addr,
   ...
   */
  // [interface 41]
  if (false && !pci_get_function_0(pci_dev)) {
    return;
  }
...
void pci_data_write(PCIBus *s, uint32_t addr, uint32_t val, int len) {
  ...
  pci_host_config_write_common(pci_dev, config_addr, PCI_CONFIG_SPACE_SIZE, val,
                               len);
}
...
static void pci_host_config_write(void *opaque, hwaddr addr, uint64_t val,
                                  unsigned len) {
...
static void pci_host_data_write(void *opaque, hwaddr addr, uint64_t val,
                                unsigned len) {
  PCIHostState *s = opaque;
