...
static void ioapic_entry_parse(uint64_t entry, struct ioapic_entry_info *info) {
  ...
  info->delivery_mode = (entry >> IOAPIC_LVT_DELIV_MODE_SHIFT) & IOAPIC_DM_MASK;
...
#ifdef CONFIG_KVM
static void ioapic_iec_notifier(void *private, bool global, uint32_t index,
                                uint32_t mask) {
...
static uint64_t ioapic_mem_read(void *opaque, hwaddr addr, unsigned int size) {
  IOAPICCommonState *s = opaque;
  int index;
  uint32_t val = 0;
  ...
  case IOAPIC_IOWIN:
    ...
    case IOAPIC_REG_ARB:
      ...
      break;
    case IOAPIC_REG_VER:
      val = s->version | ((IOAPIC_NUM_PINS - 1) << IOAPIC_VER_ENTRIES_SHIFT);
      break;
    default:
 ...
 * work correctly.
 */
static inline void ioapic_fix_edge_remote_irr(uint64_t *entry) {
...
static void ioapic_mem_write(void *opaque, hwaddr addr, uint64_t val,
                             unsigned int size) {
  IOAPICCommonState *s = opaque;
  int index;
...
#ifdef CONFIG_KVM
  IOAPICCommonState *s =
      container_of(notifier, IOAPICCommonState, machine_done);
...
static void ioapic_register_types(void) { type_register_static(&ioapic_info); }
