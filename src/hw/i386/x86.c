...
void x86_cpus_init(X86MachineState *x86ms, int default_cpu_version) {
  ...
  MachineState *ms = X86_TO_MACHINE(x86ms);
  MachineClass *mc = MACHINE_GET_CLASS(X86_TO_MACHINE(x86ms));
   ...
   */
  x86ms->apic_id_limit =
      x86_cpu_apic_id_from_index(x86ms, ms->smp.max_cpus - 1) + 1;
...
int64_t x86_get_default_cpu_node_id(const MachineState *ms, int idx) {
  X86CPUTopoInfo topo;
  X86MachineState *x86ms = MS_TO_X86_MACHINE(ms);

  assert(idx < ms->possible_cpus->len);
  x86_topo_ids_from_apicid(ms->possible_cpus->cpus[idx].arch_id,
                           x86ms->smp_dies, ms->smp.cores, ms->smp.threads,
                           &topo);
...
static uint64_t read_pvh_start_addr(void *arg1, void *arg2, bool is64) {
  ...
  if (is64) {
    ...
    elf_note_data_addr =
        ((void *)nhdr64) + nhdr_size64 + QEMU_ALIGN_UP(nhdr_namesz, phdr_align);
  } else {
    ...
    elf_note_data_addr =
        ((void *)nhdr32) + nhdr_size32 + QEMU_ALIGN_UP(nhdr_namesz, phdr_align);
  }
...
static bool load_elfboot(const char *kernel_filename, int kernel_file_size,
                         uint8_t *header, size_t pvh_xen_start_addr,
                         FWCfgState *fw_cfg) {
  uint32_t flags = 0;
  ...
  flags = elf_is64 ? ((Elf64_Ehdr *)header)->e_flags
                   : ((Elf32_Ehdr *)header)->e_flags;
  ...
  kernel_size =
      load_elf(kernel_filename, read_pvh_start_addr, NULL, &elf_note_type,
               &elf_entry, &elf_low, &elf_high, 0, I386_ELF_MACHINE, 0, 0);
...
void x86_load_linux(X86MachineState *x86ms, FWCfgState *fw_cfg,
  ...
  if (!kernel_size || fread(header, 1, MIN(ARRAY_SIZE(header), kernel_size),
                            f) != MIN(ARRAY_SIZE(header), kernel_size)) {
  ...
  if (ldl_p(header + 0x202) == 0x53726448) {
     ...
     */
    if (pvh_enabled && load_elfboot(kernel_filename, kernel_size, header,
                                    pvh_start_addr, fw_cfg)) {
      fclose(f);
      ...
      fw_cfg_add_bytes(fw_cfg, FW_CFG_SETUP_DATA, header, sizeof(header));
...
void x86_bios_rom_init(MemoryRegion *rom_memory, bool isapc_ram_fw) {
  ...
  memory_region_add_subregion_overlap(rom_memory, 0x100000 - isa_bios_size,
                                      isa_bios, 1);
  if (!isapc_ram_fw) {
...
void x86_machine_class_init(X86MachineClass *x86mc) {
  ...
  object_class_property_set_description(
      oc, X86_MACHINE_MAX_RAM_BELOW_4G,
...
static const TypeInfo x86_machine_info = {
    ...
    .interfaces = (InterfaceInfo[]){{TYPE_NMI}, {}},
};
