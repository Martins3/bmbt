...
static const char *cc_op_str[CC_OP_NB] = {
    "DYNAMIC", "EFLAGS", "MULB",   "MULW",   "MULL",   "MULQ", "ADDB",
    "ADDW",    "ADDL",   "ADDQ",   "ADCB",   "ADCW",   "ADCL", "ADCQ",
    "SUBB",    "SUBW",   "SUBL",   "SUBQ",   "SBBB",   "SBBW", "SBBL",
    "SBBQ",    "LOGICB", "LOGICW", "LOGICL", "LOGICQ", "INCB", "INCW",
    "INCL",    "INCQ",   "DECB",   "DECW",   "DECL",   "DECQ", "SHLB",
    "SHLW",    "SHLL",   "SHLQ",   "SARB",   "SARW",   "SARL", "SARQ",
    "BMILGB",  "BMILGW", "BMILGL", "BMILGQ", "ADCX",   "ADOX", "ADCOX",
    "CLR",
};
...
static void cpu_x86_dump_seg_cache(CPUX86State *env, FILE *f, const char *name,
  ...
  if (env->hflags & HF_CS64_MASK) {
    qemu_fprintf(f, "%-3s=%04x %016" PRIx64 " %08x %08x", name, sc->selector,
                 sc->base, sc->limit, sc->flags & 0x00ffff00);
  } else
#endif
  {
  ...
  if (sc->flags & DESC_S_MASK) {
    if (sc->flags & DESC_CS_MASK) {
      ...
      qemu_fprintf(f, (sc->flags & DESC_B_MASK || env->hflags & HF_LMA_MASK)
                          ? "DS  "
                          : "DS16");
...
void x86_cpu_dump_local_apic_state(CPUState *cs, int flags) {
  ...
  if (s->dest_mode == 0) {
    qemu_printf("(cluster %u: id %u)", s->log_dest >> APIC_LOGDEST_XAPIC_SHIFT,
...
void x86_cpu_dump_state(CPUState *cs, FILE *f, int flags) {
  ...
  if (env->hflags & HF_CS64_MASK) {
    qemu_fprintf(
        ...
        env->regs[8], env->regs[9], env->regs[10], env->regs[11], env->regs[12],
        env->regs[13], env->regs[14], env->regs[15], env->eip, eflags,
        eflags & DF_MASK ? 'D' : '-', eflags & CC_O ? 'O' : '-',
        eflags & CC_S ? 'S' : '-', eflags & CC_Z ? 'Z' : '-',
        eflags & CC_A ? 'A' : '-', eflags & CC_P ? 'P' : '-',
        eflags & CC_C ? 'C' : '-', env->hflags & HF_CPL_MASK,
        (env->hflags >> HF_INHIBIT_IRQ_SHIFT) & 1, (env->a20_mask >> 20) & 1,
  ...
  if (env->hflags & HF_LMA_MASK) {
    ...
    qemu_fprintf(f, "CR0=%08x CR2=%016" PRIx64 " CR3=%016" PRIx64 " CR4=%08x\n",
                 (uint32_t)env->cr[0], env->cr[2], env->cr[3],
                 (uint32_t)env->cr[4]);
    for (i = 0; i < 4; i++)
  ...
  if (flags & CPU_DUMP_FPU) {
    ...
    qemu_fprintf(f, "FCW=%04x FSW=%04x [ST=%d] FTW=%02x MXCSR=%08x\n",
                 env->fpuc, (env->fpus & ~0x3800) | (env->fpstt & 0x7) << 11,
                 env->fpstt, fptag, env->mxcsr);
    for (i = 0; i < 8; i++) {
      CPU_LDoubleU u;
      u.d = env->fpregs[i].d;
      qemu_fprintf(f, "FPR%d=%016" PRIx64 " %04x", i, u.l.lower, u.l.upper);
      if ((i & 1) == 1)
        qemu_fprintf(f, "\n");
      else
        qemu_fprintf(f, " ");
    ...
    if (env->hflags & HF_CS64_MASK)
      ...
      nb = 8;
    for (i = 0; i < nb; i++) {
      qemu_fprintf(f, "XMM%02d=%08x%08x%08x%08x", i, env->xmm_regs[i].ZMM_L(3),
                   env->xmm_regs[i].ZMM_L(2), env->xmm_regs[i].ZMM_L(1),
