                                   ...
                                   int is_hw)
{
    ...
    env->xtm_flags.is_in_int   += 1;
    env->xtm_flags.is_int_inst = is_int;
    if (env->xtm_flags.is_in_int == 1) {
        /* This is the first interrupt in nested interrupt */
        env->xtm_flags.is_top_int_inst = is_int;
    }
#endif
#if defined(CONFIG_SOFTMMU) && defined(CONFIG_XTM_PROFILE)
...
#if defined(CONFIG_SOFTMMU) && defined(CONFIG_XTM_PROFILE)
    if (seg_reg == R_SS) {
        if (env->xtm_pf_data.tbf.is_mov)
            xtm_pf_inc_tbf_mov_ss();
        if (env->xtm_pf_data.tbf.is_pop)
            xtm_pf_inc_tbf_pop_ss();
    }
#endif
