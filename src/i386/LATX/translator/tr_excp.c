...
void tr_gen_raise_exception(IR1_INST *ir1, int excp_index, int end);
void tr_gen_raise_exception_addr(ADDRX addr, int excp_index, int end);
...
int tr_gen_excp_check(IR1_INST *pir1)
{
    ...
    switch (opc) {
        ...
        case X86_INS_CLI:
        case X86_INS_STI:
            if (!td->sys.vm86)  EC_CPLLEIOPL_GP_AND_RETURN();
            else                EC_IOPLEQ_GP_AND_RETURN(3);
            break;
        /* Mov instruction :
        ...
        case X86_INS_MOV:
            ...
            if (ir1_opnd_is_cr(opnd0) || ir1_opnd_is_cr(opnd1)) {
                EC_CPLEQ_GP_AND_RETURN(0);
            }
            /* mov to/from debug register */
            if (ir1_opnd_is_dr(opnd0) || ir1_opnd_is_dr(opnd1)) {
                EC_CPLEQ_GP_AND_RETURN(0);
            }
        ...
        case X86_INS_XSETBV:
            if (!(td->sys.cpuid_ext_features & CPUID_EXT_XSAVE) ||
                ir1_has_prefix_lock(pir1)      ||
                ir1_has_prefix_opsize(pir1)    ||
                ir1_has_prefix_repe(pir1)      ||
...
void tr_gen_raise_exception(IR1_INST *ir1, int excp_index, int end)
{
    /* 1. save the address of the instruction caused exception
     *    into eip */
    ADDRX ir1_eip = ir1_addr(ir1);
    IR2_OPND eip_reg = ra_alloc_itemp();
    load_addrx_to_ir2(&eip_reg, ir1_eip);
    append_ir2_opnd2i(LISA_ST_W, &eip_reg, &env_ir2_opnd,
    ...
    tr_sys_gen_call_to_helper2_cfg(
...
void tr_gen_raise_exception_addr(ADDRX addr, int excp_index, int end)
{
    ...
    IR2_OPND eip_reg = ra_alloc_itemp();
    load_addrx_to_ir2(&eip_reg, addr);
    append_ir2_opnd2i(LISA_ST_W, &eip_reg, &env_ir2_opnd,
                      lsenv_offset_of_eip(lsenv));
...
{
    tr_gen_raise_exception_addr(addr, EXCP06_ILLOP, end);
}
void tr_gen_excp_illegal_op(IR1_INST *ir1, int end)
{
    /* #define EXCP06_ILLOP	6 */
    tr_gen_raise_exception(ir1, EXCP06_ILLOP, end);
}

void tr_gen_excp_gp(IR1_INST *ir1, int end)
{
    /* #define EXCP0D_GPF	13 */
    tr_gen_raise_exception(ir1, EXCP0D_GPF, end);
}

void tr_gen_excp_prex(IR1_INST *ir1, int end)
{
    /* #define EXCP07_PREX	7 */
    tr_gen_raise_exception(ir1, EXCP07_PREX, end);
}

void tr_gen_excp_divz(IR1_INST *ir1, int end)
{
    /* #define EXCP00_DIVZ	0 */
    tr_gen_raise_exception(ir1, EXCP00_DIVZ, end);
}
...
int tb_encode_search(TranslationBlock *tb, uint8_t *block)
{
    TRANSLATION_DATA *td = lsenv->tr_data;
    uint8_t *p = block;
    int i, j, n;
    ...
    for (i = 0, n = tb->icount; i < n; ++i) {
        ...
        for (j = 0; j < TARGET_INSN_START_WORDS; ++j) {
            ...
            if (i == 0) {
                ...
                prev_addr = ir1_addr(td->ir1_inst_array + i - 1) + td->sys.cs_base;
                prev_data = (j == 0 ? prev_addr : 0);
            }

            this_addr = ir1_addr(td->ir1_inst_array + i) + td->sys.cs_base;
            this_data = (j == 0 ? this_addr : 0);

            p = encode_sleb128(p, this_data - prev_data);
            if (option_dump)
                fprintf(stderr,"[%3d][%d] = %#x\n", i, j, this_data - prev_data);
        }

        prev_data = (i == 0 ? 0 : td->x86_ins_mips_nr[i - 1]*4);
        this_data = td->x86_ins_mips_nr[i] * 4;
        p = encode_sleb128(p, this_data - prev_data);
        if (option_dump)
            fprintf(stderr,"[%3d]OFF = %#x\n", i, this_data);
    }
